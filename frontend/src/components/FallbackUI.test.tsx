...
describe('FallbackUI', () => {
  describe('FallbackUI Component', () => {
    ...
    it('renders server type correctly', () => {
      ...
      expect(screen.getByText('The server is currently unavailable. Our team has been notified and is working on a fix.')).toBeInTheDocument()
    })
    ...
    it('renders auth type correctly', () => {
      ...
      expect(screen.getByText('You need to sign in to access this feature. Please log in and try again.')).toBeInTheDocument()
    })
    ...
    it('renders data type correctly', () => {
      ...
      expect(screen.getByText('Unable to load the requested data. This might be a temporary issue.')).toBeInTheDocument()
    })
    ...
    it('renders component type correctly', () => {
      ...
      expect(screen.getByText('This component encountered an error and cannot be displayed properly.')).toBeInTheDocument()
    })
  ...
  describe('Specialized Fallback Components', () => {
    it('NetworkErrorFallback renders correctly', () => {
      ...
      expect(screen.getByText('Connection Lost')).toBeInTheDocument()
      expect(screen.getByText('Please check your internet connection and try again. If the problem persists, contact your network administrator.')).toBeInTheDocument()
    ...
    it('ServerErrorFallback renders correctly', () => {
      ...
      expect(screen.getByText('Server Unavailable')).toBeInTheDocument()
      expect(screen.getByText('Our servers are temporarily unavailable. We\'re working to restore service as quickly as possible.')).toBeInTheDocument()
    })
    ...
    it('AuthErrorFallback renders correctly', () => {
      ...
      expect(screen.getByText('Authentication Required')).toBeInTheDocument()
      expect(screen.getByText('Your session has expired or you don\'t have permission to access this resource.')).toBeInTheDocument()
    })
    ...
    it('DataErrorFallback renders correctly', () => {
      ...
      expect(screen.getByText('Data Unavailable')).toBeInTheDocument()
      expect(screen.getByText('Unable to load the requested information. This might be due to a temporary data issue.')).toBeInTheDocument()
    })
  })
