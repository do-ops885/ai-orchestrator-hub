...
async fn main() -> anyhow::Result<()> {
    ...
    neural_processor.create_neural_agent(
        basic_agent,
        "general".to_string(),
        false, // Use basic NLP
    ).await?;
    ...
    #[cfg(not(feature = "advanced-neural"))]
    {
        ...
        println!("📝 Created agent (advanced features not enabled): {}", advanced_agent);
    }
    ...
    for (scenario, expected_success) in test_scenarios {
        ...
        let basic_performance = neural_processor.predict_performance(basic_agent, scenario).await?;
        let advanced_performance = neural_processor.predict_performance(advanced_agent, scenario).await?;
    ...
    if let Some(basic_perf) = neural_processor.get_agent_performance(basic_agent) {
        println!("📝 Basic Agent Overall Performance: {:.2}%", basic_perf * 100.0);
    }
    
    if let Some(advanced_perf) = neural_processor.get_agent_performance(advanced_agent) {
        println!("🚀 Advanced Agent Overall Performance: {:.2}%", advanced_perf * 100.0);
    }
