...
async fn demo_encryption_compression() -> anyhow::Result<()> {
    ...
    let config = PersistenceConfig {
        ...
        encryption_key: Some(
            "4f8a9b2c3d1e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f01".to_string(),
    ...
    for i in 0..5 {
        ...
        info!(
            "Added agents batch {}, total agents: {}",
            i + 1,
            hive.agents.len()
        );
    }
    ...
    persistence
        ...
        .await?;
    info!(
        "Successfully restored from encrypted checkpoint with {} agents",
    ...
    for (i, size) in checkpoint_sizes.iter().enumerate() {
        ...
        let checkpoint_id = persistence
            .create_checkpoint(
                &hive,
                Some(format!(
                    "Statistics demo checkpoint {} ({} agents)",
                    i + 1,
                    hive.agents.len()
                )),
            )
        ...
        info!(
            "Created checkpoint {} with {} agents",
            i + 1,
            hive.agents.len()
        );
...
async fn demo_performance_comparison() -> anyhow::Result<()> {
    ...
    let configs = vec![
        ...
        (
            "Fast compression, no encryption",
            PersistenceConfig {
                storage_backend: StorageBackend::Memory { max_snapshots: 10 },
                checkpoint_interval_minutes: 1,
                max_snapshots: 10,
