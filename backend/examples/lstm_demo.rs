...
async fn main() -> Result<()> {
    ...
    for (i, (task, success)) in tasks.iter().enumerate() {
        ...
        // Train both agents
        processor
            .learn_from_interaction_adaptive(basic_agent, task, "task completed", *success)
            .await?;

        processor
            .learn_from_interaction_adaptive(lstm_agent, task, "task completed", *success)
            .await?;
    }
    ...
    let basic_processed = processor
        .process_text_adaptive(sample_text, basic_agent)
        .await?;
    let lstm_processed = processor
        .process_text_adaptive(sample_text, lstm_agent)
        .await?;
    ...
    println!(
        "  • Vector magnitude: {:.3}",
        basic_processed.semantic_vector.magnitude
    );
    ...
    println!(
        "  • Vector magnitude: {:.3}",
        lstm_processed.semantic_vector.magnitude
    );
