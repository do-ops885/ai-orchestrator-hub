...
async fn main() -> Result<()> {
    ...
    let training_scenarios = vec![
        ...
        ("Recognize speech patterns", "Speech recognition successful", true),
        ("Identify fraud patterns", "Transaction flagged as suspicious", false),
        ("Classify text sentiment", "Positive sentiment detected", true),
        
        // Forecasting tasks
        ("Predict stock movement", "Stock price increased", true),
        ("Forecast weather patterns", "Rain predicted correctly", true),
        ("Estimate project timeline", "Project completed on time", true),
        ("Predict user behavior", "User engagement increased", false),
        ("Forecast demand", "Demand exceeded prediction", false),
        
        // General tasks
        ("Process customer request", "Request handled successfully", true),
    ...
    println!("  Training {} scenarios across all agents...", training_scenarios.len());
    ...
    for (i, (task, output, success)) in training_scenarios.iter().enumerate() {
        ...
        processor.learn_from_interaction_adaptive(
            basic_agent, task, output, *success
        ).await?;
        
        processor.learn_from_interaction_adaptive(
            fann_agent, task, output, *success
        ).await?;
        
        processor.learn_from_interaction_adaptive(
            lstm_agent, task, output, *success
        ).await?;
    ...
    for (i, text) in sample_texts.iter().enumerate() {
        ...
        println!("    Basic  - Sentiment: {:.3}, Keywords: {}", 
                basic_processed.sentiment, basic_processed.keywords.len());
        println!("    FANN   - Sentiment: {:.3}, Keywords: {}", 
                fann_processed.sentiment, fann_processed.keywords.len());
        println!("    LSTM   - Sentiment: {:.3}, Keywords: {}", 
                lstm_processed.sentiment, lstm_processed.keywords.len());
    }
    ...
    for task in &["pattern recognition", "forecasting", "classification", "prediction"] {
    ...
    println!("    FANN:      {:.3} ({:+.3} vs Basic)", fann_avg, fann_avg - basic_avg);
    println!("    LSTM:      {:.3} ({:+.3} vs Basic)", lstm_avg, lstm_avg - basic_avg);
