...
async fn run_comprehensive_test_suite() {
    ...
    println!(
        "Configuration: timeout={}ms, max_agents={}, max_tasks={}",
...
async fn test_system_initialization() {
    ...
    assert_eq!(
        ...
        0,
        "Hive should start with no agents"
    );
    ...
    assert_eq!(
        status["metrics"]["total_agents"], 0,
        "Initial agent count should be 0"
    );
}
...
async fn test_agent_lifecycle() {
    ...
    // Test agent creation
    let agent_config =
        create_agent_config("TestAgent", "worker", Some(vec![("general", 0.7, 0.1)]));
...
async fn test_hive_coordination() {
    ...
    assert!(
        swarm_center.0.is_finite() && swarm_center.1.is_finite(),
...
async fn test_performance_scalability() {
    ...
    let status = hive.get_status().await;
    assert_eq!(
        status["metrics"]["total_agents"], num_agents,
        "Metrics should reflect agent count"
    );

    // Check resource utilization
    let resource_info = hive.get_resource_info().await;
    assert!(
        resource_info["system_resources"]["cpu_usage"].is_number(),
        "Should report CPU usage"
    );
    assert!(
        resource_info["system_resources"]["memory_usage"].is_number(),
...
async fn test_error_handling() {
    ...
    assert!(
        agent_result.is_ok(),
        "Should handle invalid agent config gracefully"
    );
    ...
    assert!(
        task_result.is_ok(),
        "Should handle invalid task config gracefully"
    );
    ...
    assert_eq!(
        ...
        "Empty hive should have no agents"
    );
...
async fn benchmark_system_performance() {
    ...
    assert!(
        agent_creation_time.as_millis() < 5000,
        "Agent creation should be fast"
    );
    assert!(
        task_creation_time.as_millis() < 5000,
...
async fn test_memory_usage() {
    ...
    let resource_info = hive.get_resource_info().await;
    let memory_usage = resource_info["system_resources"]["memory_usage"]
        .as_f64()
        .unwrap();
