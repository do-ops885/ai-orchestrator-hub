...
impl MessagePool {
    ...
    pub fn get_stats(&self) -> MessagePoolStats {
        ...
        MessagePoolStats {
            ...
            reused_count: reused,
            reuse_ratio: if created == 0 { 0.0 } else { reused as f64 / created as f64 },
...
impl BatchProcessor {
    ...
    pub async fn add_message(&self, target: String, message: MessageEnvelope) -> HiveResult<()> {
        ...
        let pending_batch = pending.entry(target.clone()).or_insert_with(|| PendingBatch {
            messages: Vec::new(),
    ...
    async fn send_batch(&self, pending_batch: PendingBatch) -> HiveResult<()> {
        ...
        stats.average_batch_size = f64::midpoint(
            stats.average_batch_size,
            message_count as f64,
        );
        stats.average_batch_time_ms = f64::midpoint(
...
impl OptimizedMessagingStats {
    ...
    pub fn calculate_efficiency_metrics(&self) -> EfficiencyMetrics {
        ...
        EfficiencyMetrics {
            ...
            overall_efficiency_score: (bandwidth_savings + memory_efficiency + batching_efficiency) / 3.0 * 100.0,
        }
    }
}
...
mod tests {
    ...
    async fn test_optimized_communicator() {
        ...
        let result = communicator.send_optimized_message("test".to_string(), message).await;
        assert!(result.is_ok());
