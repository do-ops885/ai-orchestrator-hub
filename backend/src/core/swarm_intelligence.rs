...
impl SwarmIntelligenceEngine {
    ...
    pub async fn optimize_formation(
        &mut self,
        agents: &[Agent],
        task: &Task,
        ...
        debug!(
            "Created formation {} with {} agents",
            formation.formation_id,
            formation.agents.len()
        );
        self.formations
            .insert(formation.formation_id, formation.clone());
        Ok(formation)
    }
    ...
    fn determine_optimal_formation(
        ...
        debug!(
            "Selected formation type: {:?} for task complexity: {}",
            adjusted_formation, complexity_factor
        );
    ...
    async fn select_optimal_agents(
        ...
        let capable_agents: Vec<&Agent> = agents
            .iter()
        ...
        let max_agents = self
            .optimization_config
            .max_agents_per_formation
            .min(capable_agents.len());
        ...
        if selected.len() < self.optimization_config.min_agents_per_formation {
            ...
            for agent in capable_agents
                .iter()
                .take(self.optimization_config.min_agents_per_formation)
            {
    ...
    fn select_star_formation(&self, agents: &[&Agent]) -> anyhow::Result<Vec<Uuid>> {
        ...
        } else if let Some(best_agent) = agents.iter().max_by(|a, b| {
            self.calculate_coordination_score(a)
                .partial_cmp(&self.calculate_coordination_score(b))
        ...
        // Add workers
        let worker_ids: Vec<Uuid> = agents
            .iter()
    ...
    fn select_hierarchy_formation(&self, agents: &[&Agent]) -> anyhow::Result<Vec<Uuid>> {
        ...
        } else if let Some(leader) = agents
            .iter()
            .max_by(|a, b| a.memory.experiences.len().cmp(&b.memory.experiences.len()))
        {
            selected.push(leader.id);
        ...
        // Select sub-leaders
        let sub_leader_ids: Vec<Uuid> = agents
            .iter()
            .filter(|a| !selected.contains(&a.id))
            .filter(|a| {
                matches!(a.agent_type, AgentType::Specialist(_)) || a.memory.experiences.len() > 50
            })
            .take(2)
            .map(|a| a.id)
            .collect();
    ...
    pub fn update_formation_performance(&mut self, formation_id: Uuid, performance_score: f64) {
        if let Some(formation) = self.formations.get_mut(&formation_id) {
            ...
            info!(
                "Updated formation {} performance to {}",
                formation_id, performance_score
            );
        }
    }
    ...
    pub fn record_agent_performance(&mut self, agent_id: Uuid, performance: f64) {
        ...
        debug!(
            "Recorded performance {} for agent {}",
    ...
    pub fn rebalance_formations(&mut self, agents: &[Agent]) -> anyhow::Result<Vec<Uuid>> {
        ...
        let underperforming: Vec<Uuid> = self
            .formations
            .iter()
            .filter(|(_, formation)| {
                formation.efficiency_score < self.optimization_config.efficiency_threshold
            })
            .map(|(id, _)| *id)
            .collect();
        ...
        for formation_id in underperforming {
            if let Some(formation) = self.formations.get(&formation_id) {
                ...
                let current_agents: Vec<&Agent> = agents
                    .iter()
