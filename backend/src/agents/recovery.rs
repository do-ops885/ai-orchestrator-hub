...
impl AgentRecoveryManager {
        ...
        while attempts < self.max_retry_attempts {
            match self.attempt_recovery(agent).await {
                Ok(()) => {
                    ...
                    info!("Successfully recovered agent {} after {} attempts", agent.id, attempts + 1);
                    return Ok(());
                }
                Err(e) => {
                    attempts += 1;
                    warn!("Recovery attempt {} failed for agent {}: {}", attempts, agent.id, e);
                    
                    if attempts >= self.max_retry_attempts {
                        error!("Recovery failed for agent {} after {} attempts", agent.id, attempts);
    ...
    async fn validate_agent_health(&self, agent: &Agent) -> HiveResult<()> {
        ...
        if agent.position.0.is_nan() || agent.position.1.is_nan() {
            return Err(HiveError::AgentExecutionFailed("Invalid position".to_string()));
        }
        ...
        for capability in &agent.capabilities {
            if capability.proficiency < 0.0 || capability.proficiency > 1.0 {
                return Err(HiveError::AgentExecutionFailed(
                    format!("Invalid capability proficiency: {}", capability.proficiency)
                ));
            }
        }
    ...
    pub fn can_recover(&self, agent: &Agent) -> bool {
        ...
        match agent.state {
            ...
            AgentState::Idle | AgentState::Working | AgentState::Learning | AgentState::Communicating => false,
        }
    }
    ...
    pub async fn diagnose_agent(&self, agent: &Agent) -> Vec<String> {
        ...
        for (i, capability) in agent.capabilities.iter().enumerate() {
            if capability.proficiency < 0.0 || capability.proficiency > 1.0 {
                issues.push(format!("Invalid proficiency in capability {}: {}", i, capability.proficiency));
            }
            if capability.learning_rate < 0.0 {
                issues.push(format!("Invalid learning rate in capability {}: {}", i, capability.learning_rate));
            }
        }
...
mod tests {
    use super::*;
    use crate::agents::agent::{AgentType, AgentCapability};
    use uuid::Uuid;
