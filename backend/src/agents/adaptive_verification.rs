...
use crate::agents::simple_verification::{
    SimpleVerificationSystem, SimpleVerificationResult, SimpleVerificationStatus
};
...
impl AdaptiveVerificationSystem {
    ...
    pub async fn adaptive_verify_task_result(
        ...
        let verification_result = self.base_verification
        ...
        self.learning_system.write().await
    ...
    async fn get_current_threshold_recommendation(&self) -> Result<ThresholdRecommendation> {
        ...
        let recent_outcomes = self.get_recent_outcomes(&performance, Duration::hours(self.adaptation_config.adaptation_window_hours as i64));
        ...
        let optimal_confidence_threshold = self.calculate_optimal_confidence_threshold(&recent_outcomes).await;
        
        // Calculate optimal rule thresholds
        let optimal_rule_thresholds = self.calculate_optimal_rule_thresholds(&recent_outcomes).await;
        
        // Estimate performance improvement
        let current_performance = self.calculate_current_performance_score(&recent_outcomes);
        let expected_performance = self.estimate_performance_with_thresholds(
            &recent_outcomes,
        ...
        let recommendation_confidence = self.calculate_recommendation_confidence(
            recent_outcomes.len(),
    ...
    async fn calculate_optimal_confidence_threshold(&self, outcomes: &[&VerificationOutcome]) -> f64 {
        ...
        for threshold_test in (50..=95).step_by(5) {
            ...
            let score = self.evaluate_threshold_performance(outcomes, threshold).await;
    ...
    /// Calculate optimal rule thresholds
    async fn calculate_optimal_rule_thresholds(&self, outcomes: &[&VerificationOutcome]) -> HashMap<String, f64> {
        ...
        for rule_id in rule_ids {
            let optimal_threshold = self.calculate_optimal_rule_threshold(outcomes, &rule_id).await;
    ...
    async fn calculate_optimal_rule_threshold(&self, outcomes: &[&VerificationOutcome], rule_id: &str) -> f64 {
        ...
        for threshold_test in (30..=90).step_by(10) {
            let threshold = threshold_test as f64 / 100.0;
            let score = self.evaluate_rule_threshold_performance(outcomes, rule_id, threshold).await;
    ...
    async fn evaluate_threshold_performance(&self, outcomes: &[&VerificationOutcome], threshold: f64) -> f64 {
    ...
    async fn evaluate_rule_threshold_performance(&self, outcomes: &[&VerificationOutcome], rule_id: &str, threshold: f64) -> f64 {
        ...
        self.evaluate_threshold_performance(&relevant_outcomes.into_iter().copied().collect::<Vec<_>>(), threshold).await
    }
    ...
    async fn apply_threshold_recommendation(&mut self, recommendation: &ThresholdRecommendation) {
        ...
        self.base_verification.configure(recommendation.confidence_threshold);
        ...
        debug!("Applied adaptive thresholds: confidence={:.3}, rules={:?}", 
               recommendation.confidence_threshold, 
               recommendation.rule_threshold_adjustments);
    }
    ...
    async fn record_verification_outcome(
        ...
        self.update_accuracy_metrics(&mut tracker, verification_result, actual_success).await;
    ...
    async fn update_accuracy_metrics(
        ...
        tracker.accuracy_metrics.recall = if tp + fn_count > 0.0 { tp / (tp + fn_count) } else { 0.0 };
    ...
    async fn should_adapt_thresholds(&self) -> bool {
        ...
        time_since_last_adaptation >= Duration::hours(self.adaptation_config.adaptation_frequency_hours as i64)
    }
    ...
    async fn adapt_thresholds(&self) -> Result<()> {
        ...
        if recommendation.confidence_in_recommendation >= 0.7 && 
           recommendation.expected_performance_improvement > 0.01 {
            ...
            info!("Adapted verification thresholds: confidence={:.3}, expected_improvement={:.2}%, reason={}",
                  recommendation.confidence_threshold,
                  recommendation.expected_performance_improvement * 100.0,
                  recommendation.reasoning);
        } else {
            debug!("Skipped threshold adaptation: insufficient confidence ({:.2}) or improvement ({:.2}%)",
    ...
    fn calculate_current_performance_score(&self, outcomes: &[&VerificationOutcome]) -> f64 {
        ...
        let accuracy = outcomes.iter()
            .map(|outcome| {
                ...
                if verification_passed == outcome.actual_task_success { 1.0 } else { 0.0 }
            })
            .sum::<f64>() / outcomes.len() as f64;
        
        let efficiency = outcomes.iter()
    ...
    async fn estimate_performance_with_thresholds(
        ...
        self.evaluate_threshold_performance(outcomes, confidence_threshold).await
    }

    fn calculate_recommendation_confidence(&self, sample_count: usize, performance_improvement: f64) -> f64 {
    ...
    fn calculate_outcome_score(&self, verification_result: &SimpleVerificationResult, actual_success: bool) -> f64 {
        let verification_passed = matches!(
            verification_result.verification_status,
            SimpleVerificationStatus::Passed | SimpleVerificationStatus::PassedWithIssues
        );
        ...
        let correctness_score = if verification_passed == actual_success { 1.0 } else { 0.0 };
    ...
    pub async fn get_adaptation_insights(&self) -> AdaptationInsights {
        ...
        AdaptationInsights {
            ...
            next_adaptation_due: history.last_adaptation + Duration::hours(self.adaptation_config.adaptation_frequency_hours as i64),
        }
    }
}
...
impl AdaptiveVerificationCapable for Agent {
    async fn adaptive_verify(
        ...
        adaptive_system.adaptive_verify_task_result(task, result, original_goal, self).await
    }
}