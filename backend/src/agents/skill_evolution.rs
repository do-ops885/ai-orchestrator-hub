...
pub enum LearningCurve {
    ...
    SCurve { initial_rate: f64, peak_rate: f64, plateau_threshold: f64 },
}
...
pub enum LearningTrigger {
    ...
    TaskFailureThreshold { failure_rate: f64, skill_category: String },
...
impl SkillEvolutionSystem {
    ...
    pub async fn start_skill_evolution(&self, agents: Arc<dashmap::DashMap<Uuid, Agent>>) {
        ...
        tokio::spawn(async move {
            let mut interval = tokio::time::interval(
                tokio::time::Duration::from_secs(config.evaluation_interval_minutes * 60)
            );
        ...
        info!("Skill evolution system started with {} minute intervals", interval_minutes);
    }
        ...
        for agent_entry in agents.iter() {
            ...
            for policy in policies.iter() {
                ...
                if Self::should_trigger_learning(&agent, policy, &library).await? {
                    info!("Learning policy '{}' triggered for agent {}", policy.name, agent_id);

                    // Select skill to learn
                    if let Some(skill_to_learn) = Self::select_skill_to_learn(
                        &agent,
                        policy,
                        &library,
                        nlp_processor,
                        ...
                        let learning_event = LearningEvent {
                            ...
                            proficiency_after: learning_result.as_ref().map(|r| r.new_proficiency).unwrap_or(0.0),
                            learning_time_hours: learning_result.as_ref().map(|r| r.learning_time).unwrap_or(0.0),
                            energy_cost: learning_result.as_ref().map(|r| r.energy_cost).unwrap_or(0.0),
                        ...
                        if let Ok(result) = learning_result {
                            ...
                            if let Some(mut agent_ref) = agents.get_mut(&agent_id) {
                                ...
                                info!("Agent {} learned new skill with proficiency {:.2}", 
                                      agent_id, result.new_proficiency);
                            }
                        } else {
                            warn!("Skill learning failed for agent {}: {:?}", agent_id, learning_result);
                        }
        ...
        for trigger in &policy.triggers {
            match trigger {
                LearningTrigger::TaskFailureThreshold { failure_rate, .. } => {
                    ...
                    let recent_failures = agent.memory.experiences.iter()
                        .rev()
                        .take(10)
                ...
                LearningTrigger::PerformancePlateau { stagnation_period_hours } => {
                ...
                LearningTrigger::PeerComparison { skill_gap_threshold } => {
    ...
    async fn select_skill_to_learn(
        ...
        let existing_skills: Vec<String> = agent.capabilities.iter()
        ...
        match &policy.skill_selection {
            SkillSelectionStrategy::AddressWeaknesses => {
                ...
                for (skill_id, template) in &library.skill_templates {
                    if !existing_skills.contains(skill_id) {
                        ...
                        if template.prerequisites.iter().all(|prereq| existing_skills.contains(prereq)) {
            ...
            SkillSelectionStrategy::BuildOnStrengths => {
                ...
                let strongest_skill = agent.capabilities.iter()
                    ...
                    if let Some(template) = library.skill_templates.get(&strong_skill.name) {
                        for related_skill in &template.related_skills {
                            if !existing_skills.contains(related_skill) {
                                if let Some(related_template) = library.skill_templates.get(related_skill) {
                                    if related_template.prerequisites.iter().all(|prereq| existing_skills.contains(prereq)) {
            ...
            SkillSelectionStrategy::RandomExploration { .. } => {
                ...
                let available_skills: Vec<String> = library.skill_templates.keys()
    ...
    async fn execute_skill_learning(
        ...
        let template = library.skill_templates.get(skill_id)
        ...
        let new_proficiency = match &template.learning_curve {
            ...
            LearningCurve::Exponential { initial_rate, decay_factor } => {
                let progress = 1.0 - (-decay_factor * learning_time).exp();
                template.base_proficiency + (template.max_proficiency - template.base_proficiency) * progress * initial_rate
            }
            LearningCurve::Logarithmic { base_rate, acceleration } => {
                template.base_proficiency + base_rate * (1.0 + learning_time * acceleration).ln()
            }
            LearningCurve::SCurve { initial_rate, peak_rate, plateau_threshold } => {
    ...
    fn create_default_policies() -> Vec<EvolutionPolicy> {
        vec![
            EvolutionPolicy {
                ...
                triggers: vec![
                    LearningTrigger::TaskFailureThreshold {
                        failure_rate: 0.3,
            ...
            EvolutionPolicy {
                ...
                triggers: vec![
                    LearningTrigger::ScheduledLearning { interval_hours: 48.0 }
                ],
    ...
    pub async fn get_evolution_stats(&self) -> serde_json::Value {
        ...
        let skills_learned = history.iter()
            .filter(|e| matches!(e.event_type, LearningEventType::SkillAcquisition))
            .count();

        let skill_popularity: HashMap<String, usize> = history.iter()
...
impl SkillLibrary {
    ...
    pub fn create_default() -> Self {
        ...
        // Define basic skills
        skill_templates.insert("communication".to_string(), SkillTemplate {
        ...
        skill_templates.insert("problem_solving".to_string(), SkillTemplate {
        ...
        skill_templates.insert("task_optimization".to_string(), SkillTemplate {
        ...
        skill_categories.insert("social".to_string(), SkillCategory {
        ...
        skill_categories.insert("cognitive".to_string(), SkillCategory {
