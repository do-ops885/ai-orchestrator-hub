...
mod tests {
    ...
    fn test_agent_types() {
        ...
        let specialist = Agent::new("Specialist".to_string(), AgentType::Specialist("AI".to_string()));
    ...
    fn test_agent_social_connections() {
        ...
        assert_approx_eq(
            *agent.memory.social_connections.get(&other_agent_id).unwrap(),
            0.6, // 0.5 + 0.1
            0.001
        );
    ...
    fn test_agent_task_fitness_calculation() {
        ...
        let expected_fitness = (0.8 + 0.6) / 2.0; // Average of capabilities
        assert_approx_eq(agent.calculate_task_fitness(&multi_req_task), expected_fitness, 0.001);
    }
    ...
    fn test_agent_energy_management() {
        ...
        assert_eq!(agent.energy, 150.0); // No automatic clamping in basic agent
        
        agent.energy = -10.0; // Below minimum
        assert_eq!(agent.energy, -10.0); // No automatic clamping in basic agent
