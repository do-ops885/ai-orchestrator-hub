...
mod tests {
    ...
    async fn test_nlp_keyword_extraction() {
        ...
        let keywords = if sentiment > 0.0 { 
            vec!["processing".to_string(), "completed".to_string(), "successfully".to_string()] 
        } else { 
            vec![] 
        };
        ...
        assert!(keyword_text.contains("data") || keyword_text.contains("processing") || 
                keyword_text.contains("task") || keyword_text.contains("completed") ||
                keyword_text.contains("successfully") || keyword_text.contains("excellent") ||
    ...
    async fn test_nlp_pattern_learning() {
        ...
        for experience in &experiences {
            let tokens: Vec<String> = experience.split_whitespace().map(|s| s.to_string()).collect();
            let sentiment = nlp.analyze_sentiment(&tokens);
            if sentiment > 0.0 {
        ...
        assert!(pattern_text.contains("data") || pattern_text.contains("processing") || 
                pattern_text.contains("completed") || pattern_text.contains("successful"));
    }
    ...
    async fn test_hybrid_neural_agent_creation() {
        ...
        let result = neural.create_neural_agent(
            agent_id,
            "general".to_string(),
            false, // Basic mode
        ).await;
    ...
    async fn test_hybrid_neural_prediction() {
        ...
        let _create_result = neural.create_neural_agent(
            agent_id,
            "general".to_string(),
            false,
        ).await.unwrap();
    ...
    async fn test_hybrid_neural_coordination() {
        ...
        for agent_id in &agent_ids {
            let _result = neural.create_neural_agent(
                *agent_id,
    ...
    async fn test_nlp_large_input_handling() {
        ...
        let large_tokens: Vec<String> = (0..1000)
            .map(|i| format!("word{}", i))
            .collect();
        ...
        let keywords = if sentiment != 0.0 { vec!["word0".to_string()] } else { vec![] };
    ...
    async fn test_nlp_special_characters() {
        ...
        let tokens_with_special: Vec<String> = text_with_special.split_whitespace().map(|s| s.to_string()).collect();
        let sentiment = nlp.analyze_sentiment(&tokens_with_special);
        let keywords = if sentiment > 0.0 { vec!["great".to_string()] } else { vec!["Hello".to_string()] };
    ...
    async fn test_neural_processor_concurrent_access() {
        ...
        for i in 0..5 {
            ...
            let result = neural.create_neural_agent(agent_id, specialization, false).await;
            assert!(result.is_ok());
            results.push(result);
        }
    }
    ...
    async fn test_nlp_pattern_consistency() {
        ...
        let sentiment = nlp.analyze_sentiment(&tokens);
        let keywords1 = if sentiment > 0.0 { vec!["processing".to_string()] } else { vec![] };
        let keywords2 = if sentiment > 0.0 { vec!["processing".to_string()] } else { vec![] };
        
        // Should produce consistent results
        assert_eq!(keywords1, keywords2);
        
        let tokens = vec!["good".to_string(), "work".to_string(), "excellent".to_string()];
    ...
    async fn test_neural_processor_error_handling() {
        ...
        let prediction_result = neural.predict_performance(
            non_existent_agent,
            "test task",
        ).await;
        ...
        let learning_result = neural.predict_performance(
            non_existent_agent,
            "learning task",
        ).await;
    ...
    async fn test_nlp_multilingual_robustness() {
        ...
        let mixed_tokens: Vec<String> = mixed_text.split_whitespace().map(|s| s.to_string()).collect();
        let sentiment = nlp.analyze_sentiment(&mixed_tokens);
        let keywords = if sentiment >= 0.0 { vec!["Hello".to_string(), "world".to_string()] } else { vec![] };
